...
class PmHostSvc extends IPluginHost.Stub {
    ...
    private void syncPluginInfo2All(PluginInfo pi) {
        ...
        // 3. 同步到各进程，这里存在两种情况：
        //    a) 同步的是PluginInfoNew，则所有进程的内存表都强制更新到新的Info上，因此【正在运行的】插件信息将丢失，会出现严重问题
        //    b) 同步的是PluginInfoOld，只不过这个Old里面有个mPendingUpdate指向PendingInfoNew，则不会有问题，旧的仍被使用，符合预期
        // 4. 最终返回出去的一定是PluginInfoNew，这样最外面的RePlugin.install拿到的就是安装成功的插件信息，符合预期
